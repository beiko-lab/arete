...
def plot_distance_histogram(distance_dict, save_path):
    ...
    # Set histogram title, axis labels, and ticks
    ax.set_title(
        "Average distance score obtained for AMR gene neighborhood across genomes"
    )
...
def graph_UPGMA_clusters(linkage_matrix, labels, AMR_gene, output_path):
    ...
    hierarchy.dendrogram(
        Z=linkage_matrix, leaf_rotation=90, leaf_font_size=8, labels=labels
    )
    savename = os.path.join(output_path + "/clustering/UPGMA/", AMR_gene + ".png")
    plt.title(
        "UPGMA dendrogram for gene {g} across {n} genomes".format(
            g=AMR_gene, n=len(labels)
        )
    )
...
def plotly_dendrogram(linkage_matrix, labels, AMR_gene, output_path):
    ...
    fig = create_dendrogram(
        linkage_matrix, labels=labels, colorscale=get_colors(len(labels))
    )
    fig.update_layout(
        autosize=True,
        title=title,
...
def draw_mcl_graph(matrix, clusters, **kwargs):
    ...
    savename = os.path.join(
        kwargs["save_path"] + "/clustering/MCL/", kwargs["save_name"] + ".png"
    )
    plt.savefig(savename, bbox_inches="tight", dpi=100)
...
def plotly_mcl_network(matrix, clusters, genome_names, AMR_gene, output_path):
    ...
    for i, cluster in enumerate(clusters):
        for node in cluster:
            ...
            node_text[node] = "{g}: Cluster {c}".format(
                g=node_to_genome_id_map[node], c=cluster_map[node]
            )
        ...
        if cluster_map[node] not in legend_clusters:
            fig.add_trace(
                go.Scatter(
                    x=[x_pos],
                    y=[y_pos],
                    name=str(cluster_map[node]),
                    mode="markers",
                    hoverinfo="text",
                    text=[node_text[node]],
                    line=dict(width=2, color="black"),
                    legendgroup=str(cluster_map[node]),
                    legendrank=cluster_map[node],
                    marker=dict(
                        color=cluster_colors_dict[str(cluster_map[node])], size=15
                    ),
                )
            )
    ...
    fig.update_layout(
        legend_title="MCL Cluster", paper_bgcolor="white", template="plotly_white"
    )
...
def graph_DBSCAN_clusters(
    distance_matrix_df, DBSCAN_clusters, labels, AMR_gene, output_path
):
    """
    ...
    scatterplot = ax.scatter(
        x=pcoa_vals.samples["PC1"],
        y=pcoa_vals.samples["PC2"],
        c=labels,
        cmap=cmap,
        norm=norm,
    )
    ...
    for i in range(len(distance_matrix_df.index)):
        plt.text(
            pcoa_vals.samples.loc[str(i), "PC1"],
            pcoa_vals.samples.loc[str(i), "PC2"],
...
def plotly_pcoa(distance_matrix_df, genome_ids, labels, AMR_gene, output_path):
    ...
    fig.update_layout(
        paper_bgcolor="white", template="plotly_white", width=419, height=316
    )
