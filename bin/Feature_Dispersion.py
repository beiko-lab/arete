...
def calculate_feature_counts(feature_file):
    ...
    for feature_col in feature_df.columns[1:]:  # Exclude 'genome' column
        # Collect the genomes where this feature is present into a comma-separated list
        genomes_with_feature = feature_df[feature_df[feature_col] == 1][
            "genome"
        ].tolist()
    ...
    features_df = pd.DataFrame(
        {"feature": features, "total_count": total_counts, "genomes_list": genomes_list}
    )
...
def verify_genome_ids(tree, feature_file, samplesheet_file=None):
    ...
    if samplesheet_file:
        ...
        samplesheet_genomes = set(samplesheet_df["genome"])
    else:
    ...
    if missing_in_tree:
        print(
            f"Error: The following genome IDs are missing in the phylogenetic tree: {', '.join(missing_in_tree)}"
        )
        exit(1)
...
def generate_heatmap(output_df, output_heatmap):
    ...
    genome_bins = np.linspace(
        0, max_genome_count, 11
    )  # Generate 11 edges to create 10 bins
    genome_bins_labels = [
        f"{int(genome_bins[i]) + 1}-{int(genome_bins[i + 1])}"
        for i in range(len(genome_bins) - 1)
    ]
    ...
    sns.heatmap(
        ...
        cmap="Reds",
        cbar_kws={"label": "Number of Features (log scale)"},
        annot=heatmap_data,
        fmt="g",
        linewidths=0.5,
    )
    ...
    plt.yticks(
        ticks=np.arange(0.5, len(genome_bins_labels), 1), labels=genome_bins_labels
    )
...
def main(
    ...
    if samplesheet_file:
        ...
        if "genome" not in available_columns:
            print("Error: 'genome' column is missing in the samplesheet.")
            exit(1)

        samplesheet_data = {}
        for column in samplesheet_columns:
            if column in available_columns:
                samplesheet_data[column] = (
                    samplesheet_df[["genome", column]]
                    .set_index("genome")[column]
                    .to_dict()
                )
            else:
                print(
                    f"Warning: Column '{column}' not found in the samplesheet. Skipping."
                )
    else:
        samplesheet_data = {}
        ...
        if len(genomes_list) > 1:
            ...
            for column in samplesheet_columns:
                if column in samplesheet_data:
                    ...
                    distinct_values = set(
                        samplesheet_data[column].get(genome, None)
                        for genome in genomes_list
                        if genome in samplesheet_data[column]
                    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Calculate feature statistics based on phylogenetic tree and genus information."
    )
    parser.add_argument(
        "--tree_file", type=str, required=True, help="Path to the Newick tree file"
    )
    parser.add_argument(
        "--feature_file",
        type=str,
        required=True,
        help="Path to the feature presence/absence file",
    )
    parser.add_argument(
        "--output_base",
        type=str,
    ...
    parser.add_argument(
        "--samplesheet_file",
        type=str,
        help="Path to the file mapping genome IDs to other properties",
    )
    parser.add_argument(
        "--samplesheet_columns",
        type=str,
        help="Comma-separated list of columns to process from the samplesheet",
    )
