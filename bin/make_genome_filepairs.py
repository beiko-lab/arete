...
def parse_args(args=None):
    ...
    Epilog = (
        "Example usage: python make_genome_filepairs.py <ASSEMBLY_PATH> <OUTPUT_PATH>"
    )
    ...
    parser.add_argument(
        ...
        metavar="output_path",
        type=str,
        help="Path to output directory where " "CSV file will be outputted.",
    )
...
def make_filepairs_csv(assembly_path, output_path):
    ...
    full_genome_paths = [
        os.path.abspath(assembly_path) + "/" + file
    ...
    for genome in full_genome_paths:
        ...
        row = [
            g1 + "_" + g1,
            genome,
            os.path.abspath(output_path) + "/diamond/" + db_filename + ".dmnd",
        ]
        row_data.append(row)
