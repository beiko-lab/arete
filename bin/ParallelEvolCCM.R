...
parse_args <- function(args) {
  ...
  if (length(args) == 0 || any(args == "-h")) {
    cat("Usage: RunEvolCCM.R --intree <tree_file> --intable <table_file> [--compare_from value1,value2,...] [--compare_to valueA,valueB,...] [--cores <number_or_-1>] [--min_abundance <0.0-1.0>] [--max_abundance <0.0-1.0>]\n")
    cat("Options:\n")
    cat("  --intree <tree_file>        : Path to the tree file (required).\n")
    cat("  --intable <table_file>      : Path to the table file (required).\n")
    cat("  --compare_from <values>     : Comma-separated list of values for comparison (optional).\n")
    cat("  --compare_to <values>       : Comma-separated list of values for comparison (optional).\n")
    cat("  --cores <number_or_-1>      : Specify number of cores for parallel processing or '-1' for all cores (optional, default is 1).\n")
  ...
  if (!is.null(min_abundance_str)) {
    ...
    if (!is.na(temp_val) && temp_val >= 0.0 && temp_val <= 1.0) {
      ...
      stop("--min_abundance must be a proportional value between 0.0 and 1.0", call. = FALSE)
    }
  }
  ...
  if (!is.null(max_abundance_str)) {
    ...
    if (!is.na(temp_val) && temp_val >= 0.0 && temp_val <= 1.0) {
      ...
      stop("--max_abundance must be a proportional value between 0.0 and 1.0", call. = FALSE)
    }
  }

  # Check the relationship between min_abund and max_abund
  if (min_abund > max_abund) {
    stop("--min_abundance value must be less than or equal to --max_abundance value", call. = FALSE)
  }
  ...
  if (is.null(cores_str)) {
    ...
    if (is.na(num_cores) || num_cores <= 0) {
      stop("--cores argument must be a positive integer or '-1' for all cores", call. = FALSE)
    }
  }
  ...
  if (is.null(inputTree) || is.null(inputProfile)) {
    stop("Usage: RunEvolCCM.R --intree <tree_file> --intable <table_file> [--compare_from value1,value2,...] [--compare_to valueA,valueB,...]", call. = FALSE)
  }

  parsed_list <- list("inputTree" = inputTree,
...
modify_tree <- function(tree,fix_multifurcations,coerce_branches) {

  if (!is.rooted(tree)) {
    warning("Input tree is not rooted. Applying midpoint rooting algorithm.")
  ...
  if (!is.binary(tree)) {
    if (fix_multifurcations) {
      warning("Input tree is not binary! Forcing the issue.")
      tree<-multi2di(tree)
    } else {
      stop("Tree contains multifurcations and \"fix_multifurcations\" is set to FALSE. Exiting...")
    }
  }
  ...
  if (coerce_branches) {
    ...
    if (short_branch_count > 0) {
      notification_string = paste(short_branch_count, " of ",nrow(tree$edge)," edges are less than the specified threshold of ",min_branch_length,".", sep = "")
      warning(paste(notification_string,"Setting these branches to equal the threshold; this will affect the results!"))
...
check_and_filter_profile <- function(aprofile,tree,min_abund,max_abund) {
  ...
  if (length(missing_in_df) > 0) {
    print("Labels in the tree but not in the dataframe:",quote=FALSE)
    print(missing_in_df)
    mismatch = 2
  }
  ...
  if (length(missing_in_tree) > 0) {
    print("Labels in the dataframe but not in the tree:",quote=FALSE)
  ...
  print("All names match between tree and matrix.",quote=FALSE)
  ...
  print(paste("Total number of features: ",numFeatures),quote=FALSE)

  ### Abundance filter: subset the dataframe to include only those columns that satisfy the proportion criteria ###
  proportion_ones <- apply(aprofile, 2, function(col) sum(col == 1) / length(col))
  aprofile <- aprofile[, proportion_ones >= min_abund & proportion_ones <= max_abund]
  numFeatures <- dim(aprofile)[2]
  print(paste("Total number of features post abundance filtering: ",numFeatures),quote=FALSE)
  ...
  if(FALSE) {
    ...
    clades <- lapply((length(tree$tip.label) + 1):(length(tree$tip.label) + tree$Nnode),
                     function(node) get_descendant_tips(tree, node))
    ...
    check_feature <- function(feature){
      for(clade in clades){
        ...
        if (all(clade_data == 1, na.rm = TRUE) & all(outside_clade_data == 0, na.rm = TRUE)){
          return(TRUE)
        }
      }
      return(FALSE)
...
process_pair <- function(i, j, filtered_data, tree) {
  res <- tryCatch({
    ...
    list(i = i, j = j, colnames = colnames(filtered_data[, c(i, j)]), res = res)
    #}
  }, error = function(e) {
...
results <- foreach(i = 1:(numFeatures - 1), .combine = 'c', .multicombine = TRUE) %dopar% {
  ...
  i_condition <- ifelse(!is.null(compare_from_vector),
                        any(sapply(compare_from_vector, function(x) {startsWith(i_name, x)})),
                        TRUE)

  if (i_condition) {
    print(paste("Processing feature #", i, " (", i_name, ")", sep = ""), quote = FALSE)
    ...
    sapply((i + 1):numFeatures, function(j) {
      ...
      j_condition <- ifelse(!is.null(compare_to_vector),
                            any(sapply(compare_to_vector, function(x) {startsWith(j_name, x)})),
                            TRUE)
...
for (result in results) {
  if (!is.null(result)) {
    ...
    cat(paste(result$colnames,collapse="\t"), file = temp_output_file, append = TRUE)
    cat("\t", file = temp_output_file, append = TRUE)
    cat(paste(result$res,collapse="\t"), file = temp_output_file, append = TRUE)
